...
public:
  ...
  void PrintRegisters() const noexcept override
  {
    const std::unordered_map<RegisterName, std::string> names = {
        {RegisterName::AX, "AX"}, {RegisterName::BX, "BX"}, {RegisterName::CX, "CX"}, {RegisterName::DX, "DX"}, {RegisterName::SI, "SI"}, {RegisterName::DI, "DI"}, {RegisterName::BP, "BP"}, {RegisterName::SP, "SP"}, {RegisterName::IP, "IP"}, {RegisterName::CS, "CS"}, {RegisterName::DS, "DS"}, {RegisterName::ES, "ES"}, {RegisterName::SS, "SS"}};
  ...
  {
    std::cout << "[CF] " << static_cast<bool>(Flag{this, FlagName::Carry}) << std::endl;
    std::cout << "[PF] " << static_cast<bool>(Flag{this, FlagName::Parity}) << std::endl;
    std::cout << "[AF] " << static_cast<bool>(Flag{this, FlagName::AuxiliaryCarry}) << std::endl;
    std::cout << "[ZF] " << static_cast<bool>(Flag{this, FlagName::Zero}) << std::endl;
    std::cout << "[SF] " << static_cast<bool>(Flag{this, FlagName::Sign}) << std::endl;
    std::cout << "[TF] " << static_cast<bool>(Flag{this, FlagName::Trap}) << std::endl;
    std::cout << "[IF] " << static_cast<bool>(Flag{this, FlagName::Interrupt}) << std::endl;
    std::cout << "[DF] " << static_cast<bool>(Flag{this, FlagName::Direction}) << std::endl;
    std::cout << "[OF] " << static_cast<bool>(Flag{this, FlagName::Overflow}) << std::endl;
  }

  class Execute
  {
  public:
    Execute(Cpu8086 *t) : Cpu8086_this(t) {}

    void operator()(InstructionES &instruction)
    {
      auto &srcReg = Cpu8086_this->RegisterWord(instruction.Op<SrcReg<uint16_t>>().Register);
    ...
    void operator()(InstructionAXIv &instruction)
    {
      auto &dstReg = Cpu8086_this->RegisterWord(instruction.Op<DstReg<uint16_t>>().Register);
      auto immediate = instruction.Op<Immediate<uint16_t>>().Value;
    ...
    void operator()(InstructionALIb &instruction)
    {
      auto &dstReg = Cpu8086_this->RegisterByte(instruction.Op<DstReg<uint8_t>>().Register);
      auto immediate = instruction.Op<Immediate<uint8_t>>().Value;
    ...
    {
      auto &dstReg = Cpu8086_this->RegisterWord(instruction.Op<DstReg<uint16_t>>().Register);
      auto address = instruction.Op<Address>().Address;
    ...
    {
      auto &cs = instruction.Op<DirectAddress>().Segment;
      auto &ip = instruction.Op<DirectAddress>().Pointer;
    ...
    {
      auto &dstReg = Cpu8086_this->RegisterByte(instruction.Op<DstReg<uint8_t>>().Register);
      auto &srcReg = Cpu8086_this->RegisterByte(instruction.Op<SrcReg<uint8_t>>().Register);
...
private:
  ...
  void Add(T &dst, T &src)
  {
    ...
    Flag{this, FlagName::Overflow} = ((dst > 0) && (src > std::numeric_limits<T>::max() - dst)) || ((dst < 0) && (src < std::numeric_limits<T>::min() - dst));
  ...
  void Multiply(T &dst, T &src)
  {
    ...
    Flag{this, FlagName::Zero} = res == 0;
    Flag{this, FlagName::Sign} = res < 0;
    Flag{this, FlagName::Overflow} = (src > std::numeric_limits<T>::max() / dst) || (src < std::numeric_limits<T>::min() / dst);
    Flag{this, FlagName::Carry} = res < 0;
...
private:
  ...
  std::unordered_map<RegisterName, Register> m_Registers = {
      {RegisterName::AX, {}}, {RegisterName::BX, {}}, {RegisterName::CX, {}}, {RegisterName::DX, {}}, {RegisterName::SI, {}}, {RegisterName::DI, {}}, {RegisterName::BP, {}}, {RegisterName::SP, {}}, {RegisterName::IP, {}}, {RegisterName::CS, {}}, {RegisterName::DS, {}}, {RegisterName::ES, {}}, {RegisterName::SS, {}}, {RegisterName::Flags, {}}};

  class Flag
  {
  public:
    Flag(Cpu8086 *t, FlagName name)
        : Cpu8086_this(t), Name(name) {}
