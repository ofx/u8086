...
using InstructionEbGb = Instruction<DstReg<uint8_t>, SrcReg<uint8_t>>;

using InstructionEvGv = Instruction<DstReg<uint16_t>, SrcReg<uint16_t>>;

using Address = AddressOperand<OperandType::Address>;

using InstructionGbEb = Instruction<DstReg<uint8_t>, Address>;

using InstructionGvEv = Instruction<DstReg<uint16_t>, Address>;

using InstructionZbIb = Instruction<DstReg<uint8_t>, Immediate<uint8_t>>;

using InstructionZvIv = Instruction<DstReg<uint16_t>, Immediate<uint16_t>>;
...
template<typename T>
static T
Read(gsl::span<uint8_t> v, uint32_t& offset)
{
...
Decode(uint32_t offset, gsl::span<uint8_t> ram)
{
  ...
  switch (opcode) {
    ...
    case 0x3C:
    case 0xA8: {
      auto instruction = InstructionZbIb(opcode, 2);
    ...
    case 0xB7: {
      auto instruction = InstructionZbIb(opcode, 2);
      instruction.Op<DstReg<uint8_t>>().Register = g_decodeTable8[opcode & 0x7];
      instruction.Op<Immediate<uint8_t>>().Value = Read<uint8_t>(ram, offset);
      return instruction;
    } break;
    // AX Iv
    ...
    case 0x3D:
    case 0xA9: {
      auto instruction = InstructionZvIv(opcode, 3);
    ...
    case 0xF: {
      auto instruction = InstructionES(opcode, 1);
      instruction.Op<SrcReg<uint16_t>>().Register = RegisterName::CS;
      return instruction;
    } break;
    // SS
    case 0x16:
    case 0x17: {
      auto instruction = InstructionES(opcode, 1);
      instruction.Op<SrcReg<uint16_t>>().Register = RegisterName::SS;
      return instruction;
    } break;
    // DS
    case 0x1E:
    case 0x1F: {
      auto instruction = InstructionES(opcode, 1);
      instruction.Op<SrcReg<uint16_t>>().Register = RegisterName::DS;
      return instruction;
    } break;
  }
